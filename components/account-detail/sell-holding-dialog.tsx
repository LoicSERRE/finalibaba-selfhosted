...
export function SellHoldingDialog({ accountId, holding }: { accountId: string; holding: Holding }) {
  ...
  const holdingQty = parseFloat(holding.quantity.toString()) || 0;
  ...
  const gainCents = Math.round((parseFloat(proceeds) || 0) * 100) - Math.round((parseFloat(costBasis) || 0) * 100);
