...
export function Sidebar({ showLogout = false }: Readonly<SidebarProps>) {
  ...
  return (
    <>
      ...
      {/* ── Mobile bottom nav ── */}
      {/* Icon-only: with 7 items (since "income" was added), French labels
          like "Récurrentes"/"Paramètres" don't fit in a ~1/7-screen-width
          cell on a small phone even truncated - the truncated fragment
          ("Réc…") isn't legible/useful. Icons alone are a well-established
          mobile bottom-nav pattern that scales to any label length/locale;
          aria-label carries the full text for screen readers, and the
          active item still gets a highlighted background + bolder stroke. */}
        ...
        {navItems.map(({ href, label, icon: Icon, exact }) => {
          ...
          return (
            <Link
              ...
              aria-current={active ? "page" : undefined}
              aria-label={label}
              className={`flex-1 flex items-center justify-center py-3.5 rounded-xl transition active:scale-[0.93] active:opacity-80 focus-visible:outline-none focus-visible:bg-[var(--surface-elevated)] ${
                active
