...
**Both of the above are why `alerts-section.tsx` is a client component** (`"use client"`, like `ShareLinksSection`/`TwoFactorSection`) - the preset dropdown and generate/copy buttons need interactivity. It reads translations via `useTranslations("settings.alerts")` internally, **not** a `t` prop from the server page - `app/settings/page.tsx` used to pass `t={t}` when this was a server component, but a `getTranslations()` result is a function, not serializable across the RSC boundary, so that had to be removed when this file gained `"use client"`.
