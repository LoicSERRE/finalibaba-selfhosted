...
**Known, accepted coverage gaps**: `lib/actions/*` (all 17 Server Action files - every DB mutation in the app) and `lib/services/gocardless.ts` have zero tests; would need Prisma mocking or a real test DB, neither exists yet. `sync/` only tests the pure functions extracted from `sync_tr.py` - `sync_lcl.py`/`sync_woob.py`/`db.py`/`main.py` are untested. No component-rendering tests (no jest-dom/RTL/Playwright as committed dependencies) - components stay thin wrappers around the tested `lib/` functions by design, e.g. the dashboard's JSX extraction into `components/dashboard/dashboard-view.tsx` described under "Read-only share links" below.
...
- **Sync failures** (`checkSyncFailures`) - queries `SyncLog` for `status = "error"` rows newer than `lastSyncFailureAlertCheckedAt`. Deliberately scoped to the *automatic* 4h cron only, not on-demand "Sync now" clicks in Settings - a manual sync's failure is already visible right there in the UI to whoever just clicked it; a push notification adds value specifically for the unattended background job. Short-circuits to `[]` when `UserSettings.syncFailureAlertsEnabled` is false.
...
- **1. Self-learned history** (`lib/domain/auto-categorize.ts`'s `suggestCategoryAssignments`) - learns a `(accountId, normalizeLabelForCategorization(label)) -> categoryId` mapping from the user's own already-categorized history. This always takes priority over the other two sources when it has an answer, since it reflects this specific user's actual, confirmed intent, even when that intent disagrees with what MCC or the dictionary would have guessed - same per-account scoping as `detectCandidates` (recurring transactions, above) and the `/budgets` uncategorized-groups UI, for the identical reason (`label` is raw bank-feed text specific to one institution's formatting). Two confidence thresholds: `MIN_HISTORY_OCCURRENCES = 2` (a label needs at least 2 prior categorized occurrences before its majority category is trusted - a single manually-corrected mistake shouldn't immediately start auto-propagating) and `MIN_CONSISTENCY_RATIO = 0.7` (the majority category must clear 70% of that label's history, not just be the plurality winner of a scattered mix). Below either threshold, the transaction is simply absent from the suggestion map - never mapped to a guessed-but-unconfident category.
  - **`normalizeLabelForCategorization`** (also `lib/domain/auto-categorize.ts`) is a looser variant of `recurring.ts`'s own `normalizeLabel`, used only here and by "Correcting a categorization everywhere at once" below - recurring-transaction detection keeps its own separately-tuned `normalizeLabel` untouched. On top of the usual trim+lowercase, it strips an embedded calendar year (`\b(19|20)\d{2}\b`) - a real gap a user hit in production: French Livret interest is credited once a year with a label like "INTERETS 2025"/"INTERETS 2026", so under plain `normalizeLabel` two occurrences of the *exact same* label could never accumulate (the label itself changes every year, and interest lands only once a year) - `MIN_HISTORY_OCCURRENCES` could never be reached, so self-learning could never fire for this pattern at all. The word-boundary regex only strips an *isolated* year-like number, not a 4-digit run embedded in a longer reference/invoice token.
...
- `createIncomeEvent`/`updateIncomeEvent` (`lib/actions/income.ts`) call `assertIncomeEventEligible(accountId, type)` before writing anything - `DIVIDEND` only on `INVESTMENT`/`CRYPTO` accounts, `INTEREST` only on `CHECKING`/`SAVINGS`. This mirrors `components/income/add-income-dialog.tsx`'s client-side account filter and `assertCsvImportEligible`'s rationale: Server Actions are directly invocable regardless of what's rendered, so the eligibility rule has to be enforced server-side too, not just via which accounts populate the dialog's dropdown.
...
- Markdown export (`components/shared/export-tax-report-button.tsx`) reuses `lib/utils/markdown-export.ts`'s `fmt`/`sign`/`downloadFile`, same convention as `components/shared/export-analytics-button.tsx`/`components/shared/export-accounts-button.tsx` - this is the "country-agnostic, exportable" part of the roadmap wording; the report itself is presented as an estimate for informational purposes, not an official tax document or a literal French IFU form mapping.
