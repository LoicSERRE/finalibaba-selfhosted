...
- **1. Self-learned history** (`lib/domain/auto-categorize.ts`'s `suggestCategoryAssignments`) - learns a `(accountId, normalizeLabel(label)) -> categoryId` mapping from the user's own already-categorized history. This always takes priority over the other two sources when it has an answer, since it reflects this specific user's actual, confirmed intent, even when that intent disagrees with what MCC or the dictionary would have guessed - same `(accountId, normalizeLabel(label))` grouping key `detectCandidates` (recurring transactions, above) and the `/budgets` uncategorized-groups UI already use, for the identical "per-account, not cross-account" reason (`label` is raw bank-feed text specific to one institution's formatting). Two confidence thresholds: `MIN_HISTORY_OCCURRENCES = 2` (a label needs at least 2 prior categorized occurrences before its majority category is trusted - a single manually-corrected mistake shouldn't immediately start auto-propagating) and `MIN_CONSISTENCY_RATIO = 0.7` (the majority category must clear 70% of that label's history, not just be the plurality winner of a scattered mix). Below either threshold, the transaction is simply absent from the suggestion map - never mapped to a guessed-but-unconfident category.
...
**Correcting a categorization everywhere at once.** Fixing `setTransactionCategory` (`lib/actions/transactions.ts`) one `<select>` at a time doesn't scale when a dictionary/MCC/self-learning miss hit every occurrence of a label at once (the Sparplan bug above being the motivating real case) - the user explicitly asked for this after finding it painful in practice. `setTransactionCategory` now also counts how many *other* transactions in the same account share the same `normalizeLabel(label)` but currently sit in a different category (including uncategorized), returning `{ siblingCount }`. `TransactionCategorySelect` (`components/shared/transaction-category-select.tsx`) only opens a confirmation dialog when that count is `> 0` - a label with no siblings never interrupts the user with an empty prompt. Confirming calls `applyCategoryToSimilarTransactions(transactionId, categoryId)`, which re-derives the label match server-side from the source transaction (not client-supplied ids, same trust boundary as `bulkAssignCategory`) and `updateMany`s every match in that account, unconditionally overwriting whatever category (or none) they currently have - this is the one categorization path in the app that *does* touch already-categorized transactions, by design, since it's always a direct, explicit user correction rather than an automatic guess. Scoped per-account, same reasoning as every other `normalizeLabel` grouping in this codebase (label is raw bank-feed text specific to one institution's formatting).
