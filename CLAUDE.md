...
- **ntfy** - `ntfyTopicUrl`, a single POST per alert (`lib/services/notifications.ts`'s `sendNtfyMessage`). No `Title` header: HTTP header values must be Latin-1 and alert text is French/user-entered (accents), so the title goes in the body instead - a raw fetch() throws a ByteString conversion error otherwise, confirmed while building this.
