...
- `Account` (`AccountType`: `CHECKING | SAVINGS | INVESTMENT | REAL_ESTATE | MEAL_VOUCHER | CRYPTO | AUTOMOBILE | LOAN`)
  ...
  - LOAN: capital computed at runtime via `calcCurrentCapital()` from `lib/domain/loan.ts`
...
**Known accepted risk**: the sync image still has ~70 CRITICAL/HIGH findings (`perl`, `xvfb`/`xserver-common`, `libglib2.0`, `libcurl3-gnutls`, the `util-linux` family) entirely inside the Debian dependency closure that `playwright install --with-deps chromium` pulls in - none of it this project's own code. `trivy-scan` therefore runs **non-blocking** (`exit-code: 0`) for the `sync` matrix entry specifically, while staying fully blocking (`exit-code: 1`) for `app`, which is clean. This is deliberately narrower than the `find-my-way`/`valibot` exemptions under "pnpm overrides" below (those are unreachable *code paths* in an otherwise-used dependency; this is a whole browser-automation dependency closet that genuinely runs) - it's accepted because `sync/`'s HTTP API is Docker-network-only per `SECURITY.md`, never internet-exposed, not because the CVEs don't matter. Findings still upload to the Security tab either way (`if: always()` on the SARIF upload), so they stay visible - the non-blocking exit code only stops them from gating every future release until properly triaged. Reducing this further needs testing whether Playwright's actual headless Chromium launch survives trimming that dependency closure, which needs real Trade Republic credentials to verify safely and hasn't been done yet - **do not** assume any of `xvfb`/`libglib2.0`/`libcurl3-gnutls` can be dropped without testing the real `pytr` login flow (`setup_tr.py`), since headless Chromium can still depend on X11/GL-adjacent libraries for font/canvas rendering even without a real display.
