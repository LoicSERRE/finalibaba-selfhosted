...
- [X] **Native mobile app** *(scoped, decided against - see below)* - the conditional was "only if a real scoping pass finds a concrete gap the existing installable PWA doesn't already cover". It doesn't, at a price worth paying.

**Native mobile app - scoping pass and decision (not building it).**

*Checked against current sources rather than assumed, and one widely-repeated claim turned out to be false.* Several 2026 guides still list "EU: push notifications disabled, standalone mode removed, badges removed" as a live iOS restriction. That was Apple's **announced-then-reversed** plan: it was withdrawn on 1 March 2024, and home-screen web apps work normally in the EU on iOS 17.4+. Taking it at face value would have condemned this app's own primary audience (French users) to a crippled PWA and justified a native build on a premise that isn't true.

**What a native app would actually add.** Only one thing of real value: **home-screen widgets / live activities**, still unsupported for iOS PWAs in 2026 and genuinely nice for a net-worth glance. The rest of the usual list is already covered or doesn't apply here - biometric unlock is done (WebAuthn app-lock), offline is done (service worker), push is done (Web Push, iOS 16.4+), and background sync doesn't matter because this app's data is refreshed *server-side* by the `sync` container while Web Push already delivers the alerts.

**What it would cost, and the argument that decides it.** For a **self-hosted** app the distribution logic inverts. The PWA install path is "you are already on your own instance's URL - tap Add to Home Screen". A native app becomes "install from a store, then type in your server address", plus handling LAN-only hosts, self-signed certificates and VPN-only reachability - strictly more friction for the same result, and an App Store review cycle gating every update for an app with no public backend to review against. That is on top of a permanent second stack alongside Next.js. One widget does not pay for it.

**Shipped instead, from the same research**: the **Badging API** (`public/sw.js` + `service-worker-registration.tsx`), which was supported all along on iOS 16.4+ - the exact gate Web Push already requires - and simply wasn't being used. An alert now leaves a dot on the installed app icon, cleared when the app is next opened or resumed. Called with no count deliberately: this app dispatches alerts but stores no read/unread state, so any number would be invented. That closes most of the "a native app feels more present on my phone" gap for a few lines.

**Revisit if** iOS ever exposes widgets to web apps, or if real users ask for something the PWA structurally cannot do - not on general "native feels better" grounds.
